                ...
                compilationContext.RegisterOperationBlockAction(operationBlockContext =>
                {
                    ...
                    foreach (var operationRoot in operationBlockContext.OperationBlocks)
                    {
                        ...
                        if (topmostBlock != null && topmostBlock.HasAnyOperationDescendant(ShouldAnalyze))
                        {
                            ...
                            pointsToAnalysisResult = PointsToAnalysis.GetOrComputeResult(cfg, topmostBlock, containingMethod, wellKnownTypeProvider, pointsToAnalysisResult, copyAnalysisResult);
                            ...
                            foreach (var operation in cfg.DescendantOperations())
                            {
                                switch (operation.Kind)
                                {
                                    ...
                                    case OperationKind.ConditionalAccess:
                                        throw new NotImplementedException("Unexpected OperationKinds inside a ControlFlowGraph");
